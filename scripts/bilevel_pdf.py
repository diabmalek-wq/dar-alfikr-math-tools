...
for page in pdf.pages:
    for name, obj in list(page.images.items()):
        ...
        # At 460 dpi the anti-aliasing carries no information a printer can
        # resolve, so threshold to 1 bit. The glyph shape is preserved exactly
        # at full resolution — this is where the size goes, not in detail.
        #
        # But only for sparse black line art. A logo is continuous-tone and a
        # figure carries light grey fills; thresholding either wrecks it — the
        # school crest turns into a solid black square. Measure how much of the
        # image sits between the extremes: glyph anti-aliasing is a thin edge
        # and stays low, artwork and fills do not.
        h = im.histogram()
        midtone = sum(h[21:241]) / float(nw * nh)
        bits = 8
        if midtone < 0.25:
            im = im.point(lambda v: 255 if v > 176 else 0, mode="1")
            bits = 1
        ...
        data = im.tobytes()   # 1-bit packed rows, 8 px per byte
