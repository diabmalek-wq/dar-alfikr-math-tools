...
const GR11_L64 = {
  ...
  plan: {
    ...
    diagTool: [
      "5-question Quizizz ($\\le 4$ minutes): evaluate $\\log_2 8$; state the domain and range of $g(x) = 2^{x}$; find the inverse of $f(x) = x+5$; decide true or false whether $f^{-1}$ is the reflection of $f$ across $y=x$; state the horizontal asymptote of $g(x) = 3^{x}-2$.",
      "Cross-referenced against overnight EdPuzzle watch analytics — who watched, who skipped, who missed the embedded items.",
      "Answers: $\\log_2 8 = 3$; domain all reals, range $y>0$; $f^{-1}(x) = x-5$; TRUE; horizontal asymptote $y=-2$.",
    ],
    diagGap: [
    ...
    instruction: [
      ...
      "Whole class, IF.B.5 / IF.C.7.E: read $f(x)=\\log_3 x$'s key features off its graph — domain $x>0$, range all reals, $x$-intercept $(1,0)$, vertical asymptote $x=0$, end behavior $f \\to -\\infty$ as $x \\to 0$ and $f \\to \\infty$ as $x \\to \\infty$.",
      "Whole class, IF.B.6: compute the average rate of change of $f(x)=\\log_3 x$ on $[1,3]$ and on $[3,9]$ from the same graph — $(f(3)-f(1)) ÷ (3-1) = ½$, then $(f(9)-f(3)) ÷ (9-3) = ⅙$ — and name what the shrinking value shows about the curve.",
      "Whole class, BF.B.4 / (+)BF.B.4.C: the two inverse properties, $\\log_b(b^{x}) = x$ and $b^{\\log_b x} = x$, checked numerically ($\\log_2(2^{5})=5$, $3^{\\log_3 7}=7$); then read $\\log_3 9$ and $\\log_3(1⁄3)$ directly off a table of $3^{x}$ values ($x=-1,0,1,2$) without a calculator, and use both properties to explain WHY domain and range swap between a function and its inverse.",
    ...
    guided: [
      ...
      "Answers: (1) domain $x>0$, range all reals, asymptote $x=0$, $x$-intercept $(1,0)$. (2) swap: $x=\\log_2(y)+4 \\to x-4=\\log_2(y) \\to y=2^{x-4}$, so $f^{-1}(x)=2^{x-4}$.",
    ...
    independent: [
      ...
      "APPLY — Use it in context. Two inverse functions found algebraically, and a Red Sea desalination-plant pH context solved for the inverse relationship $[\\text{H}^{+}]$ in terms of pH. Done when: every inverse is solved algebraically, and the real-world answer carries units (mol/L).",
      "INVESTIGATE — Find out why. Proving $\\log_b(b^{x}) = x$ from the definition, explaining why a logarithmic function never has a $y$-intercept, and spotting the error in the claim that $\\log_b(x-h)$ and $\\log_b(x)-h$ are the same shift. Done when: you have an argument, not just an answer.",
    ...
    production: [
      ...
      "A desalination plant on the Red Sea coast measures the hydrogen-ion concentration $[\\text{H}^{+}]$ of its treated water and reports it as pH, using $\\text{pH} = -\\log_{10}[\\text{H}^{+}]$, where $[\\text{H}^{+}]$ is measured in mol/L. This monitoring supports Vision 2030 water-security targets.",
      "(a) State the domain of the pH function in terms of $[\\text{H}^{+}]$, and explain what that domain restriction means physically. (b) A sample has $[\\text{H}^{+}] = 4 \\times 10^{-8}$ mol/L. Find its pH, to two decimal places. (c) Find the inverse function — write $[\\text{H}^{+}]$ in terms of pH. (d) A technician says the inverse function is pointless, since $[\\text{H}^{+}]$ is always measured directly anyway. Explain in one sentence a situation where the inverse function is actually the useful direction.",
      "Answers: (a) $[\\text{H}^{+}]>0$ — a hydrogen-ion concentration of zero or negative is not physically possible. (b) $\\text{pH} = -\\log_{10}(4\\times10^{-8}) \\approx 7.40$. (c) $\\text{pH} = -\\log_{10}[\\text{H}^{+}] \\to -\\text{pH} = \\log_{10}[\\text{H}^{+}] \\to [\\text{H}^{+}] = 10^{-\\text{pH}}$. (d) the inverse direction is useful whenever an engineer is given a TARGET pH and needs to know what concentration to dose for, rather than measuring an existing sample.",
    ...
    evidence: [
      ...
      "Answers: 1. domain $x>-5$, range all reals, vertical asymptote $x=-5$, $x$-intercept $(-4,0)$. 2. swap: $x=\\log_4(y)-1 \\to x+1=\\log_4(y) \\to y=4^{x+1}$, so $f^{-1}(x)=4^{x+1}$. 3. the function is undefined at and beyond the asymptote's $x$-value — the domain never reaches it, so the curve can only approach, never cross.",
    ...
    exams: [
      "SAAT (Tahsili) — domain and asymptote of a shifted logarithmic function, usually asked as reading a restriction directly off the function rule. Practice tip: set the argument of the logarithm greater than zero and solve — that inequality IS the domain, every time.",
      ...
      "GAT (Qudurat) — domain restrictions on a logarithmic expression under time pressure, usually a quick single-step inequality. Practice tip: don't solve the whole function — just set the inside of the logarithm greater than zero.",
    ],
  },

  plan2026: {
    day: "Tuesday, Week 6", section: "Grade 11-B",
    ...
    phases: [
      ...
      "FIKR Phase 4 — Production. The Red Sea desalination plant's pH function: the domain restriction, the pH calculation, and the inverse function solved for $[\\text{H}^{+}]$. AI used as critic only.",
    ...
    routes: [
      ...
      { note: "Inverses, and one Saudi context", done: "your inverse is solved algebraically (not guessed), and your final concentration answer carries units (mol/L).",
        ...
        tasks: [
          ...
          "3.  A Red Sea desalination plant models water pH as $\\text{pH} = -\\log_{10}[\\text{H}^{+}]$. Find the inverse relationship, writing $[\\text{H}^{+}]$ in terms of pH.",
          "4.  Use your answer to Q3 to find $[\\text{H}^{+}]$ when pH $= 8.1$, in mol/L.",
        ], lines: 4 },
  ...
  pbl: {
    ...
    situation: [
      "A Red Sea desalination plant models water pH as $\\text{pH} = -\\log_{10}[\\text{H}^{+}]$, where $[\\text{H}^{+}]$ is the hydrogen-ion concentration in mol/L.",
    ...
    eq: "m_ctx1",
    steps: [
      ["1", "WRITE THE MODEL  (3 min)", "State the domain of the pH function in terms of $[\\text{H}^{+}]$, and explain what that restriction means physically."],
      ["2", "CONVERT FORWARD  (4 min)", "A sample has $[\\text{H}^{+}] = 4 \\times 10^{-8}$ mol/L. Find its pH, to two decimal places."],
      ["3", "FIND THE INVERSE  (5 min)", "Solve $\\text{pH} = -\\log_{10}[\\text{H}^{+}]$ for $[\\text{H}^{+}]$ — show every algebra step."],
      ["4", "CONVERT BACKWARD  (4 min)", "Use your inverse to find $[\\text{H}^{+}]$ when pH $= 8.1$ (slightly basic, typical of treated seawater)."],
