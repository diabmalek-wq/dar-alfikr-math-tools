...
// are quoted VERBATIM from the curriculum map (cmap.docx, Unit 6, Lesson 4).
// Essential Question and Mathematical Practices verified against the same
// map on 25 Sep 2026 — see the Week 6 L6-4 Rework Handoff project doc.
//
...
const GR11_L64 = {
  ...
  vocabulary: [
    ...
    { term: "Logarithmic function", def: "A function of the form $f(x)=\\log_b x$ ($b>0$, $b \\ne 1$) — the inverse of the exponential function $f(x)=b^{x}$." },
  ],
  ...
  instruction: [
    {
      ...
      sub: "Objective 1 — domain, range, intercept, asymptote, end behavior, and average rate of change",
      graph: "gm_key_features", graphW: 5.9, graphY: 2.55,
      graphAlt: "The curve f of x equals log base 3 of x, through 1 comma 0, 3 comma 1, and 9 comma 2, approaching the vertical asymptote x equals 0, with two secant lines showing the average rate of change shrinking from one half to one sixth",
      panel: {
        ...
        items: [
          ...
          "For $b>1$: as $x \\to 0$, $f(x) \\to -\\infty$; as $x \\to \\infty$, $f(x) \\to \\infty$ — a slow, never-ending climb.",
          "Average rate of change is NOT constant: from $(1,0)$ to $(3,1)$ it is ½; from $(3,1)$ to $(9,2)$ it is ⅙ — the curve keeps rising but ever more slowly.",
        ],
      },
      ...
      barY: 6.32,
      notes: "$f(x)=\\log_3 x$: domain $x>0$, range all reals, $x$-intercept $(1,0)$, vertical asymptote $x=0$, end behavior $f \\to -\\infty$ as $x \\to 0$ and $f \\to \\infty$ as $x \\to \\infty$. Average rate of change (HSF.IF.B.6): on $[1,3]$ it is $(f(3)-f(1)) ÷ (3-1) = ½$; on $[3,9]$ it is $(f(9)-f(3)) ÷ (9-3) = ⅙$ — say aloud that a shrinking average rate of change is exactly what concave down means, even though that word is not required vocabulary. Misconception to address aloud: students look for a $y$-intercept out of habit — a logarithmic function never has one, because $x=0$ is excluded from the domain, not just a point where $f$ happens to be undefined.",
    },
    {
      title: "Inverse Properties",
      sub: "Objective 2 — the two identities that prove logarithms and exponentials undo each other",
      ...
      panel: {
        h: "WHY THIS MATTERS",
        items: [
          "$\\log_b(b^{x})=x$ for every real $x$ — take the logarithm of an exponential, base and exponent vanish, leaving $x$.",
          ...
          "Domain of $\\log_b x$ is the range of $b^{x}$ (all positive reals). Range of $\\log_b x$ is the domain of $b^{x}$ (all reals) — domain and range always swap under inversion.",
        ],
      },
      panelX: 7.05, panelW: 5.85, panelH: 4.0,
      bar: ["THE TWO IDENTITIES TO MEMORIZE", "$\\log_b(b^{x}) = x$ for every real $x$.   $b^{\\log_b x} = x$ for every $x>0$."],
      barY: 6.32,
      notes: "$\\log_2(2^{5})=5$, because the logarithm and the exponential cancel. $3^{\\log_3 7}=7$, same idea in the other order. Domain of $\\log_b x$ is $x>0$, which is exactly the RANGE of $b^{x}$; range of $\\log_b x$ is all reals, which is exactly the DOMAIN of $b^{x}$. Misconception to address aloud: students think $\\log_b(b^{x})=x$ only works for \\text{nice} exponents — it holds for every real $x$, because it is an identity, not a computation.",
    },
    {
      ...
      graphAlt: "The curve g of x equals log base 3 of the quantity x minus 2, plus 1, shown beside its parent f of x equals log base 3 of x, with the vertical asymptote shifted from x equals 0 to x equals 2",
      panel: {
        ...
        items: [
          ...
          "Solve for $y$: $y = 3^{x}+1$, so $f^{-1}(x) = 3^{x}+1$.",
        ],
      },
      panelX: 7.0, panelW: 5.9, panelH: 4.0,
      bar: ["SHIFT RULE", "$\\log_b(x) + k$ shifts vertically — the asymptote stays $x=0$.   $\\log_b(x-h)$ shifts the asymptote itself, to $x=h$."],
      barY: 6.32,
      notes: "$g(x)=\\log_3(x-2)+1$ has its asymptote moved from $x=0$ to $x=2$ — the whole graph slides right 2 and up 1, exactly like any other transformation. Inverse steps: $y=\\log_3(x-1) \\to$ swap $\\to x=\\log_3(y-1) \\to$ exponential form $3^{x}=y-1 \\to y=3^{x}+1$. Misconception to address aloud: students shift the WRONG direction — $\\log_b(x-h)$ shifts the asymptote to $x=h$, not $x=-h$ — test it by substituting $x=h+1$ and checking the asymptote sits where the argument of the logarithm hits zero.",
    },
  ],
  ...
  routes: {
    mins: 8,
    tiers: [
      { items: ["State the domain, range, vertical asymptote, and $x$-intercept of four logarithmic functions: $\\log_4 x$, $\\log_2(x-3)$, $\\log_5(x)+2$, $\\log_3(x+1)-1$.", "Sketch one of the four, labelling its asymptote and $x$-intercept.", "Read the table of $3^{x}$ values below and state $\\log_3 9$ and $\\log_3(1⁄3)$ directly from it — no calculator.", "Check each answer against the general rule before moving on."],
        help: "You may use: the five-feature checklist on the board, a calculator, and a partner.",
        done: "every key feature you stated matches the general rule for its function's shift, and both table values are correct.", eq: "m_bf_table" },
      { items: ["Find the inverse of two logarithmic functions: $f(x)=\\log_2 x$, then $g(x)=\\log_5(x)-3$.", "A water-treatment lab at a Red Sea desalination plant reports pH values and needs a way to recover hydrogen-ion concentration from a pH reading, part of Vision 2030 water-security monitoring. Starting from $\\text{pH} = -\\log_{10}[\\text{H}^{+}]$, find the inverse relationship $[\\text{H}^{+}]$ in terms of pH.", "Use your inverse to find $[\\text{H}^{+}]$ when $\\text{pH}=8.1$ (slightly basic, typical of treated seawater)."],
  ...
  production: {
    ...
    situation: "A desalination plant on the Red Sea coast measures the hydrogen-ion concentration $[\\text{H}^{+}]$ of its treated water and reports it as pH, using $\\text{pH} = -\\log_{10}[\\text{H}^{+}]$, where $[\\text{H}^{+}]$ is measured in mol/L. This monitoring supports Vision 2030 water-security targets.",
    eq: "m_ctx_w", eqK: 2.0,
    tasks: [
      "(a)  State the domain of the pH function in terms of $[\\text{H}^{+}]$, and explain what that domain restriction means physically (can $[\\text{H}^{+}]$ be zero or negative?).",
      "(b)  A sample has $[\\text{H}^{+}] = 4 \\times 10^{-8}$ mol/L. Find its pH, to two decimal places.",
      "(c)  Find the inverse function — write $[\\text{H}^{+}]$ in terms of pH.",
      "(d)  A technician says the inverse function is pointless, since $[\\text{H}^{+}]$ is always measured directly anyway. Explain in one sentence a situation where the inverse function is actually the useful direction.",
    ],
  ...
  geogebra: {
    ...
    graph: "gm_exp_log_inverse",
    graphAlt: "f of x equals 3 to the x and its inverse g of x equals log base 3 of x, reflected across y equals x, with the swapped points 0 comma 1 and 1 comma 0, and 1 comma 3 and 3 comma 1, labelled",
    explore: "Graph $f(x)=b^{x}$ and $g(x)=\\log_b(x)$ together with a slider for $b$ ($b>1$) at geogebra.org/graphing. Trace a point on $f(x)$, reflect it across $y=x$, and watch it land exactly on $g(x)$ — the picture on the left is what you should see for $b=3$. Then shift $g(x)$ horizontally and watch the asymptote move with it.",
  },
  ...
  smart: {
    steps: [
      { h: "Show the model", d: "Present the pH function, the domain restriction, the pH calculation, and the inverse function solved for $[\\text{H}^{+}]$." },
      { h: "Expose the trap", d: "Add one worked NON-example — dropping the negative sign when converting pH back to $[\\text{H}^{+}]$ — and a sentence saying what error that produces (a concentration greater than 1 mol/L, which is never realistic for treated water)." },
  ...
  exams: [
    { code: "SAAT", full: "Tahsili — Achievement Test", skill: "Domain and asymptote of a shifted logarithmic function — usually a restriction read directly off the function rule.", fmt: "Four-option multiple choice, no calculator.", tip: "Set the argument of the logarithm greater than zero and solve — that inequality IS the domain, every time.",
      question: "State the vertical asymptote of $f(x)=\\log_3(x-4)$.",
      steps: ["The asymptote sits where the argument of the logarithm equals zero: $x-4=0$.", "Solve: $x=4$.", "So the vertical asymptote is $x=4$."] },
    { code: "SAT", full: "College Board", skill: "Advanced Math — writing the inverse of a logarithmic or exponential function algebraically is a recurring item type.", fmt: "Two 35-minute adaptive modules; calculator allowed throughout.", tip: "Swap $x$ and $y$ FIRST, before doing any algebra — solving for the wrong variable is the most common error on this item type.",
      ...
      steps: ["Write $y=\\log_2(x)+5$, then swap: $x=\\log_2(y)+5$.", "Isolate the logarithm: $x-5=\\log_2(y)$.", "Convert to exponential form: $y=2^{x-5}$, so $f^{-1}(x)=2^{x-5}$."] },
    { code: "GAT", full: "Qudurat — General Aptitude", skill: "Domain restrictions on a logarithmic expression under time pressure, usually a quick single-step inequality.", fmt: "Multiple choice, about 75 seconds per item, no calculator.", tip: "Don't solve the whole function — just set the inside of the logarithm greater than zero and solve that one inequality.",
      question: "Domain of $f(x)=\\log_5(x-9)$ is? (A) $x>0$  (B) $x>5$  (C) $x>9$  (D) $x \\ge 9$",
      steps: ["The argument must be strictly positive: $x-9>0$.", "Solve: $x>9$.", "Answer (C) — note it is strict, not $\\ge$, so (D) is a trap."] },
  ],
  examBar: ["THE SINGLE MOST EXAMINED IDEA HERE:", "set the argument of the logarithm greater than zero to find the domain, and swap $x$ and $y$ before you touch the algebra to find the inverse."],

  summary: [
    "Identify the domain, range, $x$-intercept, vertical asymptote, end behavior, and average rate of change of a logarithmic function.",
  ...
  notes: {
    cover: "Week 6 lesson for Grade 11-B, continuing Topic 6 (Exponential and Logarithmic Functions) directly from Lesson 6-3. Three teaching days this week, so the Smart Production step may run into the next session — protect the Mastery Gate instead.",
    objectives: "All four objectives are verbatim from the curriculum map. The map lists HSF.IF.B.5, HSF.IF.B.6, HSF.IF.C.7.E, HSF.IF.C.9, HSF.BF.B.3, HSF.BF.B.4 and the plus-standard (+)HSF.BF.B.4.C for this lesson — do not add others. HSF.IF.B.6 (average rate of change) is taught on the Key Features slide via the two secant lines; $(+)$HSF.BF.B.4.C (reading inverse values from a table) is taught on the Practice route via the table of $3^{x}$ values.",
    ...
    guided: "Answers — 1: $f(x)=\\log_3 x$ has domain $x>0$, range all reals, asymptote $x=0$, $x$-intercept $(1,0)$. 2: $f(x)=\\log_2(x)+4 \\to$ swap $\\to x=\\log_2(y)+4 \\to x-4=\\log_2(y) \\to y=2^{x-4}$, so $f^{-1}(x)=2^{x-4}$. Do not release independent work until roughly 80% have both.",
    routes: "Students choose their own route and may switch mid-task. Pear Deck flags error patterns live; use it to decide who gets a two-minute conference. Table-reading answers for the Practice route: $\\log_3 9 = 2$ (since $3^{2}=9$) and $\\log_3(1⁄3)=-1$ (since $3^{-1}=1⁄3$).",
    production: "Answers — (a) domain $[\\text{H}^{+}]>0$, since a hydrogen-ion concentration of zero or negative is not physically possible — there is always SOME concentration, however small. (b) $\\text{pH}=-\\log_{10}(4 \\times 10^{-8}) \\approx 7.40$. (c) $\\text{pH}=-\\log_{10}[\\text{H}^{+}] \\to -\\text{pH}=\\log_{10}[\\text{H}^{+}] \\to [\\text{H}^{+}]=10^{-\\text{pH}}$. (d) the inverse direction is useful whenever an engineer is given a target pH (a regulatory limit, say) and needs to know what concentration to dose FOR, rather than measuring an existing sample.",
    ...
    gate: "Answers: for $g(x)=\\log_2(x+5)$, domain $x>-5$, range all reals, vertical asymptote $x=-5$, $x$-intercept where $x+5=1$ so $x=-4$, giving $(-4,0)$; $f(x)=\\log_4(x)-1$ has inverse $f^{-1}(x)=4^{x+1}$ (swap: $x=\\log_4(y)-1 \\to x+1=\\log_4(y) \\to y=4^{x+1}$); a logarithmic graph never crosses its vertical asymptote because the function is undefined at and beyond that value — the curve approaches but the domain never reaches it. Score live and route privately.",
    smart: "The non-example in step 2 is the highest-value part — dropping the negative sign when inverting $\\text{pH}=-\\log[\\text{H}^{+}]$ gives $[\\text{H}^{+}]=10^{\\text{pH}}$ instead of $10^{-\\text{pH}}$, producing a concentration far too large to be physically real; the correction is checking the sign BEFORE trusting the number.",
